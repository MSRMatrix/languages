...
import type { Password, Storage } from "../types";
import { save } from "./save";
import clipboard from "clipboardy-ts";

export async function editPassword(passwords: Password[], data: Storage) {
  ...
  data.passwords = data.passwords.map(item =>
    item.id === selectedId
      ? { ...item, name: newNameFromUser.name }
      : item
  );

  save(data); // Ganzes Storage-Objekt speichern
  console.log("Name erfolgreich geändert!");
  return;
}

    if (passwordOption.choice === "delete") {
      data.passwords.filter(item => item.id !== selectedId);
      save(data);
