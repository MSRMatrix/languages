...
while (!passwordName) {
  if (passwordName === "") {
    console.log("Bitte gib dem Passwort einen Namen!");
  }
  ...
  passwordName = answer.name.trim();

  if (data.passwords.some(item => item.name === passwordName)) {
    console.log("Gib dem Passwort einen einmaligen Namen!");
    passwordName = "";
  }
}

// passwordName ist jetzt gültig und einzigartig
newPassword.name = passwordName;

// passwordName enthält jetzt den gültigen Namen
newPassword.name = passwordName;
